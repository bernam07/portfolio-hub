...
export default function PortfolioChart({ transactions }: Props) {
  const data = useMemo(() => {
    ...
    return sorted.map(tx => {
      cumulative += tx.amount * tx.purchasePrice;
      return {
  ...
  return (
    <div className="bg-surface p-6 rounded-2xl mb-8 w-full" style={{ height: '350px' }}>
      <h3 className="text-lg font-semibold text-textPrimary mb-6">Evolução do Investimento</h3>
        ...
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 5, right: 0, left: 0, bottom: 5 }}>
            ...
            <Tooltip 
              ...
              formatter={(value: any) => [`$${value}`, 'Investido']}
            />
            <Area 
              type="monotone" 
              dataKey="value" 
