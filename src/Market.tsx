  ...
  return (
    <div className="max-w-6xl mx-auto">
      <header className="mb-10 border-b border-surface pb-6">
        ...
        <p className="text-textSecondary mt-1">Cotações reais de Criptomoedas e Ações</p>
      </header>
      ...
      <div className="flex gap-4 mb-8">
        <button
          ...
          className={`px-6 py-2 rounded-lg font-medium transition-colors ${
            activeTab === 'crypto' ? 'bg-accent text-white' : 'bg-surface text-textSecondary hover:text-textPrimary'
          }`}
        >
          Criptomoedas
        </button>
        <button
          ...
          className={`px-6 py-2 rounded-lg font-medium transition-colors ${
            activeTab === 'stock' ? 'bg-accent text-white' : 'bg-surface text-textSecondary hover:text-textPrimary'
          }`}
        >
          Ações
        </button>
        ...
        {filteredAssets.map((asset) => {
          ...
          return (
            <Link 
              key={asset.id} 
              to={`/asset/${asset.id}`}
              className="bg-surface rounded-2xl p-5 hover:bg-bgDark border border-transparent hover:border-surface transition-all cursor-pointer block"
            >
